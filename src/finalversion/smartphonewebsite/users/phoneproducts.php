...
<div class="product" data-id="<?php echo $id?>">
					...
					<div class="caption" tabindex="0">
						<a href="product.php?model=<?php echo $model?>&id=<?php echo $id?>"><?php echo $model;?></a>
						<span><?php echo "$$minPrice - $$maxPrice";?></span>
					</div>
					<div class="info" aria-has-popup="true">
						<button tabindex="0">Quick View</button>
					</div>
					<div class="popup-hidden" role="dialog" aria-modal="true" aria-hidden="true">
						<div class="content">	
							<button class="close" aria-label="Close product">&times;</button>						
							<div class="container">
								<form method="post" action="<?php echo $_SERVER['PHP_SELF']?>" class="products">
									<div class="flex-container">
										...
										<div class="product-details">
											...
											<br>
											<p class="price-range" tabindex="0"><?php echo "$$minPrice - $$maxPrice";?></p>	
											<p class="price"></p>
										<?php 	
											if ($status === "N") {
										?>
	...
	<input type="radio" name="color" id="<?php echo "P".$id."-".$color['C_DESC'];?>" value="<?php echo $color['C_DESC'];?>" disabled>
					...
					?>
							...
							<label for="<?php echo "P".$id."-".$size['P_STG_SIZE'];?>" class="sizes"><?php echo "$size[P_STG_SIZE]";?></label>
					<?php
						}
					?>
						</div>
	...
	<label>Color: <span class="color">Please Select</span></label>
			...
			<?php
				while ($size = mysqli_fetch_assoc($sizes)) { 
			?>
					<input type="radio" name="size" id="<?php echo "P".$id."-".$size['P_STG_SIZE'];?>" value="<?php echo $size['P_STG_SIZE'];?>" data-size-id="<?php echo $size['P_STG_ID'];?>">
					<label for="<?php echo "P".$id."-".$size['P_STG_SIZE'];?>" class="sizes"><?php echo "$size[P_STG_SIZE]";?></label>
			<?php
				}
			?>
